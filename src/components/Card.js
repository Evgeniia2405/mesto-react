...
const cardDeleteButtonClassName = (
  `button element__trash-btn ${isOwn ? 'element__trash-btn_visible' : 'element__trash-btn_hidden'}`
); 
...
const cardLikeButtonClassName = (
  `button element__like-btn ${isLiked ? 'element__like-btn_active' : ''}`
);  

  return (
    <li className="element">
      <img src={props.card.link} className="element__img" alt={props.card.name} onClick={handleClick}/>
      <button type="button" className={cardDeleteButtonClassName} aria-label="Trash" onClick={handleDeleteClick}></button>
